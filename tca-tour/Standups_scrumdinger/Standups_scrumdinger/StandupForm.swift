...
struct StandupFormFeature: Reducer {
    struct State: Equatable {
        ...
        init(focus: Field? = nil, standup: Standup) {
            ...
            if self.standup.attendees.isEmpty {
                self.standup.attendees.append(Attendee(id: UUID())) // 빈 배열일 때, 기본 요소 1개 생성
            }
        }
    }
    
    enum Action: BindableAction, Equatable { // Equatable 채택!
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case let .deleteAttendees(afOffsets: indices):
        ...
        WithViewStore(self.store, observe: { $0 }) { viewStore in
            Form {
                ...
                Section {
                    ...
                    .onDelete { indices in
                        viewStore.send(.deleteAttendees(afOffsets: indices))
                    }
